...
# all functions here in the following format:
#
...
# 67: same as problem 18 but with a triangle with 100 rows (7273)
def problem67
...
# 21: find sum of amicable numbers lesser than 10,000 (31626)
def problem21
    sum amicableNumbersUnder 10000
end

# 22: What is the sum of the name scores in the following list, where score = sum of letters' alphabetical positions * position of name in sorted list? (871198282)
def problem22
...
# 25: find index of first fibonacci number to contain 1000 digits (4782)
def problem25
    firstFibTermWithNDigits 1000
end
...
# 70: minimize n/totient(n) where n in [n,10^7] and totient(n) is a permutation of n (8319823)
def problem70
...
i = 1
until found
    i1, i2, i3, i4, i5, i6 = i.to_s.split("").sort, (i*2).to_s.split("").sort, (i*3).to_s.split("").sort, (i*4).to_s.split("").sort, (i*5).to_s.split("").sort, (i*6).to_s.split("").sort
    
    puts "#{i1.count}, #{i6.count}"
    
    if i1.count == i6.count
        puts i
        match = true
        i1.count.times do |x|
            unless(i1[x] == i2[x] and i1[x] == i3[x] and i1[x] == i4[x] and i1[x] == i5[x] and i1[x] == i6[x])
                match = false
                break
            end
        end
        ...
        i = Array.new(i.to_s.length, "0").concat(["1"]).reverse.join.to_i
    end
end
...
def f n, max, done
    return 0 if done.include?(n)
    
    z = 0
    pd = properDivisors(n)
    #puts "n = #{n}\n==============\n"
    pd.sort!
    #puts "divisors: #{pd.join(' ')}"
    ...
    #if x is prime, z = 2
    if isPrime(n)
        #do all powers of this prime < max
        #if x is the kth power of prime p, z = k + 1
        b = 2
        a = n
        while(a < max)
            done << a
    ...
    #otherwise compare all appropriate divisors
    pd.count.times do |x|
        x.upto(pd.count - 1) do |y|
            z+=1 if lcm(pd[x],pd[y]) == n
        end
    end   
end
...
1.upto(6) do |x|
    ...
    puts g(10**x)
    finish = Time.now
    puts "elapsed time: #{(finish - start)} sec"
end
=end

#find largest product of 5 consecutive numbers in the provided text file
=begin
number = File.open("data.dat").readline
product, high = 0, 0
0.upto(number.length - 6) do |x|
	if !number[x..(x+4)].split("").include?("0") && (sum = number[x].to_i * number[x+1].to_i * number[x+2].to_i * number[x+3].to_i * number[x+4].to_i) > high
		puts(high = sum)
	end
end
=end
...
d = 9999
tol = 50
1.upto(99999999999999999999) do |x|
	(x+1).upto(x + tol) do |y|
		
		a = nthPentagonalNumber(x)
		b = nthPentagonalNumber(y)
		puts "x=#{x}, y=#{y}, a=#{a}, b=#{b}"
		dif = a - b
		if isPentagonal(dif)
			#puts "Difference is pentagonal."
			if isPentagonal(a+b)
				puts "\tSum and difference are pentagonal."
				if(dif<d)
					d = dif
					puts "\t\t***NEW MINIMUM: d=#{(d)}, p(i)=#{a}, p(i-1)=#{b}"
				else
					puts "\t\tLARGER: d=#{d}, p(i)=#{a}, p(i-1)=#{b}"
				end
			end
		end
		a = nthPentagonalNumber(-1.0*x)
		b = nthPentagonalNumber(y)
		dif = a - b
		if isPentagonal(dif)
			puts "Difference is pentagonal."
			if isPentagonal(a+b)
				...
				if(dif<d)
					d = dif
					puts "\t\t***NEW MINIMUM: d=#{(d)}, p(#{x})=#{a}, p(#{y})=#{b}"
				else
					puts "\t\tLARGER: d=#{d}, p(#{x})=#{a}, p(#{y})=#{b}"
				end
			end
		end
		a = nthPentagonalNumber(x)
		b = nthPentagonalNumber(-1.0*y)
		dif = a - b
		if isPentagonal(dif)
			#puts "Difference is pentagonal."
			if isPentagonal(a+b)
...
=begin
primes = primesUpto 500000
maxPrime, maxAmt = 0, 0
0.upto(primes.count) do |start|
	#prime = true
	sum = 0
	amtPrimes = 0
	i = start
	until i == primes.count do 
		#puts "adding #{primes[i]}"
		#prime = isPrime(sum += primes[i])
		sum += primes[i]
		amtPrimes += 1
		i += 1
		#puts "i = #{i}, amtPrimes = #{amtPrimes}, sum = #{sum}"
		if isPrime(sum - primes[i-1]) && amtPrimes > maxAmt
			maxPrime, maxAmt = sum - primes[i-1], amtPrimes 
			puts "sum = #{sum - primes[i-1]}, amtPrimes = #{amtPrimes}, maxPrime = #{maxPrime}, maxAmt = #{maxAmt}"
			sum, amtPrimes = 0, 0
		end
...
=begin
pan, n = false, firstFibTermWithNDigits(18)
fib1, fib2 = (fibs = fibUpto(n))[-2], fibs[-1]
amount = 0
until pan do
	newFib = fib1 + fib2
	fib1 = fib2
	fib2 = newFib
	pan = isABPandigital(newFib.to_s[0..8].to_i, 1, 9) && isABPandigital(newFib.to_s[-9..-1].to_i, 1, 9)
end
puts fib2
=end
...
#hypergeometric test
=begin
 hyp = hypgeom 0, 5, 20, 100
 print "#{hyp}\n"
=end

#hypergeometric class test
=begin
 hyp = HypergeomDist.new 20, 80
 print hyp.prob 0, 5
 print "\n"
 print hyp.mean 5
 print "\n"
