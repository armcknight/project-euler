module Mathematics
    ...
    #iterative binary gcd implementation
    def gcd u,v
...
=begin
	get the hypergeometric distribution for p out of q objects from n objects of 
	one of two dichotomous types, where m is the total amount of objects
		...
		the analogous function call for this example would be hypgeom 2, 7, 10, 50
=end
	def hypergeometricDist p, q, n, m
		return 1.0 * combination( n, p ) * combination( ( m - n ), ( q - p ) ) / combination( m, q )
	end
	...
	def isCircularPrime n
	  	return true if n == 2
		a = n.to_s.split( "" )
	  	return false if( contains( a.join, '2' ) || contains( a.join, '4' ) || contains( a.join, '6' ) || contains( a.join, '8' ) || contains( a.join, '0' ) )
...
#   [1]     ]
    ...
    def gcf a, b
        return 1 if isPrime(a) && isPrime(b)
        return a if b%a == 0
        return b if a%b == 0
        #puts intersect(primeFactors(a), primeFactors(b)).sort.last
        return intersect(primeFactors(a), primeFactors(b)).sort.last
    end
